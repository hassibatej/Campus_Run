...
void TileMap::loadFromFile(const std::string file_name)
{
	...
	if (in_file.is_open())
	{
		...
		while (in_file >> x >> y >> z >> type)
		{
			...
			else if (type == TileTypes::DEFAULT)
			{
				in_file >> trX >> trY >> collision;
			...
			else if (type == TileTypes::LEFTWALL)
			{
				in_file >> trX >> trY >> collision;
		...
		for (int y = this->fromY; y < this->toY; y++)
		{
			for (size_t k = 0; k < this->map[x][y][this->layer].size(); k++)
			{
					...
					)
				{
					...
					{
						if (this->map[x][y][this->layer][k]->getType() == TileTypes::KILLING) {
							this->dead = true;
						}
						if (this->map[x][y][this->layer][k]->getType() == TileTypes::ISFLAG) {
							this->isflag = true;
						}
						if (this->map[x][y][this->layer][k]->getType() == TileTypes::LEFTWALL) {
							entity->stopVelocityY();
